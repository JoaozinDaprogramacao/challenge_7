...
public class DepoimentoService {
	...
	public ResponseEntity<DepoimentoGetDto> add(String pessoaNome, String depoimento, byte[] bytes) {
        ...
        String uri = ServletUriComponentsBuilder
                .fromCurrentRequest()
                .path("/{id}")
                .buildAndExpand(depoimentoEntity.getId())
                .toUriString();

        return ResponseEntity.created(URI.create(uri)).body(new DepoimentoGetDto(dto.foto(), dto.nomePessoa(), dto.depoimento()));
    }
	...
	@Transactional
	public ResponseEntity<?> updateElementById(long id, DepoimentoPutDto dto) {
		if(!repository.existsById(id)) {
			return ResponseEntity.notFound().build();
		}
